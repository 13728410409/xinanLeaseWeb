<template>
  <popup v-model="currentValue">
    ...
    <div class="addresBox" ref="scroll">
      <ul>
        <li
          v-for="(item,index) of addressSelect"
          :class="{'select':item.name == addressVal[addressIndex]}"
          @click="selectClick(item.value,item.name)"
        >{{item.name}}</li>
      </ul>
    </div>
  </popup>
  ...
  methods: {
    ...
    getAddress() {
      ...
      if (addressVal) {
        ...
        async("china_address.json", {}, "GET", "", "/static/other/").then(
          data => {
